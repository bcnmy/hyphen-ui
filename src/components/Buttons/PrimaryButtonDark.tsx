...
export const PrimaryButtonDark: React.FC<IPrimaryButtonDarkProps> = ({
  ...
  return (
    <button
      className={twMerge(
        "px-4 py-2 bg-white bg-opacity-10 shadow-md rounded-full text-white text-opacity-75 border border-opacity-10 border-white ml-auto font-normal relative backdrop-blur-md cursor-pointer hover:bg-opacity-[0.15] hover:text-opacity-90 hover:border-opacity-20 hover:shadow-lg transition disabled:cursor-not-allowed",
        className
      )}
      {...props}
    >
